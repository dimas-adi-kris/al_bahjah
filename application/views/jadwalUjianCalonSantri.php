...
<div class="card">
  <div class="card-header">
    <h3 class="card-title">Data Jadwal Ujian Calon Santri</h3>
    <button id="btn-tambah-jadwal-ujian-calon-santri" type="button" class="btn btn-success float-right" data-toggle="modal" data-target="#modal_form_jadwal_ujian_calon_santri"><i class="fa fa-plus-square" aria-hidden="true"></i> Jadwal Ujian Calon Santri</button>
  </div>
  <!-- /.card-header -->
  <div class="card-body">
    <table id="tabel-list-jadwal-ujian-calon-santri" class="table table-bordered table-striped">
      <thead>
        <tr>
          <th>No</th>
          <th>ID Jadwal Ujian Calon Santri</th>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Tambah Jadwal Ujian Calon Santri</h5>
...
<div class="modal fade" id="modal_form_jadwal_ujian_calon_santri_ubah" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Ubah Jadwal Ujian Calon Santri</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
  $(document).ready(function() {
    var tableJadwalUjianCalonSantri = $('#tabel-list-jadwal-ujian-calon-santri').DataTable({
      "paging": true,
      "lengthChange": true,
      "searching": false,
      "ordering": true,
    ...
    renderTableListJadwalUjianCalonSantri();

    function renderTableListJadwalUjianCalonSantri() {
      tableJadwalUjianCalonSantri.clear();
      $.ajax({
          method: "POST",
          url: "<?= base_url() ?>index.php/JadwalUjianCalonSantri/getJadwalUjianCalonSantri",
          data: {}
        })
        .done(function(msg) {
          ...
          for (i = 0; i < res.length; i++) {
            ...
            tableJadwalUjianCalonSantri.row.add([
              i + 1,
    ...
    $("#btn-tambah-jadwal-ujian-calon-santri").click(function() {
      ...
      $('#exampleModalLabel').html('Tambah Jadwal Ujian Calon Santri');
      $('#id_calon_santri').val('');
      $('#id_jadwal_ujian').val('');
    ...
    $("#form-tambah-jadwal-ujian-calon-santri").submit(function() {
      ...
      $.ajax({
          method: "POST",
          url: "<?= base_url() ?>index.php/JadwalUjianCalonSantri/tambahJadwalUjianCalonSantri",
          data: formData
        })
        .done(function(msg) {
          console.log(msg);
    ...
    $('#tabel-list-jadwal-ujian-calon-santri').on('click', '.class_hapus_data', function() {
      ...
      if (confirm('Anda Yakin Menghapus Data')) {
        ...
        $.ajax({
            method: "POST",
            url: "<?= base_url() ?>index.php/JadwalUjianCalonSantri/hapusJadwalUjianCalonSantri",
            data: {
    ...
    $('#tabel-list-jadwal-ujian-calon-santri').on('click', '.class_ubah_data', function() {
      ...
      $.ajax({
          method: "POST",
          url: "<?= base_url() ?>index.php/JadwalUjianCalonSantri/getJadwalUjianCalonSantriById",
          data: {
            id_jadwal_ujian_calon_santri: id_jadwal_ujian_calon_santri
          }
        })
        .done(function(msg) {
          ...
          $("#form-tambah-jadwal-ujian-calon-santri-ubah").submit(function() {
            ...
            $.ajax({
                method: "POST",
                url: "<?= base_url() ?>index.php/JadwalUjianCalonSantri/updateJadwalUjianCalonSantri",
                data: formData
              })
              .done(function(msg) {
                // console.log("TES::" + msg);
